...
def parse_args():
    ...
    parser.add_argument(
        "--model-path",
        type=str,
        help="Path to the model (required for lm, multimodal, image-generation, image-edit, embeddings, whisper model types). With `image-generation` or `image-edit` model types, it should be the local path to the model.",
    )
    parser.add_argument(
        "--model-type",
        type=str,
        default="lm",
        choices=["lm", "multimodal", "image-generation", "image-edit", "embeddings", "whisper"],
        help="Model type",
    )
    parser.add_argument(
        "--context-length",
    ...
    parser.add_argument(
        "--max-concurrency", type=int, default=1, help="Maximum number of concurrent requests"
    )
    parser.add_argument("--queue-timeout", type=int, default=300, help="Request timeout in seconds")
    parser.add_argument(
        "--queue-size", type=int, default=100, help="Maximum queue size for pending requests"
    )
    parser.add_argument(
        "--quantize",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["flux-schnell", "flux-dev", "flux-krea-dev", "flux-kontext-dev"],
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable automatic tool choice. Only works with language models (`lm` or `multimodal` model types).",
    )
    parser.add_argument(
        "--tool-call-parser",
        type=str,
        default=None,
        choices=["qwen3", "glm4_moe", "qwen3_moe", "qwen3_next", "qwen3_vl", "harmony", "minimax"],
        help="Specify tool call parser to use instead of auto-detection. Only works with language models (`lm` or `multimodal` model types). Available options: `qwen3`, `glm4_moe`, `qwen3_moe`, `qwen3_next`, `qwen3_vl`, `harmony`, `minimax`.",
    )
    parser.add_argument(
        "--reasoning-parser",
        type=str,
        default=None,
        choices=["qwen3", "glm4_moe", "qwen3_moe", "qwen3_next", "qwen3_vl", "harmony", "minimax"],
        help="Specify reasoning parser to use instead of auto-detection. Only works with language models (`lm` or `multimodal` model types). Available options: `qwen3`, `glm4_moe`, `qwen3_moe`, `qwen3_next`, `qwen3_vl`, `harmony`, `minimax`.",
    )
    parser.add_argument(
        "--trust-remote-code",
        action="store_true",
...
async def setup_server(args) -> uvicorn.Config:
    ...
    return uvicorn.Config(
        app=app, host=args.host, port=args.port, log_level="info", access_log=True
    )

...
async def _get_handler_or_error(
    ...
    if handler_manager is not None:
        try:
            ...
            logger.exception(
                f"Unable to load handler via JIT for {reason}. {type(exc).__name__}: {exc}"
            )
            return (
                None,
                JSONResponse(
        ...
        async for chunk in generator:
            ...
            if isinstance(chunk, str):
                ...
                if payload.get("name"):
                    ...
                    current_tool_id = get_tool_call_id()
                    tool_call_ids[tool_call_index] = current_tool_id
                elif payload.get("arguments") and "index" not in payload:
                    payload["index"] = tool_call_index

                if current_tool_id is None and "index" in payload:
                    current_tool_id = tool_call_ids.get(payload["index"])

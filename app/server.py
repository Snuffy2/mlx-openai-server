...
class IdleAutoUnloadController:
    ...
    async def _wait_for_event(self) -> None:
        ...
        allowing the background task to wait for activity notifications.
        """
    ...
    async def _watch_loop(self) -> None:
        ...
        Runs continuously while active, checking for idle timeouts every
        10 seconds. When the idle timeout is exceeded, automatically
        unloads the current handler to free memory.
        """
        try:
            while self._active:

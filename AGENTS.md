...
- **Forward references**: Use string literals for forward references when necessary to avoid circular imports.
...
- **Docstrings required**: Every module, class, method, and function must have a descriptive docstring following PEP 257 conventions.
- **Docstring format**: Use triple double-quotes. The first line should be a concise summary; additional lines may include parameter descriptions, return values, and raised exceptions.
...
def calculate_metrics(data: list[float], threshold: float) -> dict[str, float]:
    ...
    Raises:
        ValueError: If data list is empty or threshold is negative.
    """
    ...
```
